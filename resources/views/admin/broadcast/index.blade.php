...
<div x-data="broadcastApp()" x-init="initApp()" class="max-w-7xl mx-auto space-y-6 lg:space-y-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div class="flex items-center gap-3 flex-wrap" x-cloak>
            <div x-show="gatewayStatus === 'CONNECTED'" class="flex items-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-700 rounded-md font-bold text-sm border border-emerald-200">
                <div class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>Gateway Online
            </div>
            <div x-show="gatewayStatus === 'DISCONNECTED' || gatewayStatus === 'OFFLINE'" class="flex items-center gap-2 px-4 py-2 bg-rose-50 text-rose-600 rounded-md font-bold text-sm border border-rose-200">
                <div class="w-2 h-2 rounded-full bg-rose-500"></div>Gateway Offline
            </div>
            <div x-show="gatewayStatus === 'WAITING_FOR_SCAN'" class="flex items-center gap-2 px-4 py-2 bg-amber-50 text-amber-600 rounded-md font-bold text-sm border border-amber-200">
                <div class="w-2 h-2 rounded-full bg-amber-500 animate-ping"></div>Waiting Scan
            </div>
            <div x-show="gatewayStatus === 'RECONNECTING'" class="flex items-center gap-2 px-4 py-2 bg-purple-50 text-purple-600 rounded-md font-bold text-sm border border-purple-200">
            ...
            <button x-show="gatewayStatus === 'CONNECTED'" @click="disconnectGateway"
                class="px-4 py-2 bg-white border border-rose-200 text-rose-600 font-extrabold text-[10px] uppercase tracking-widest rounded-md hover:bg-rose-50 transition-colors shadow-sm">
                Disconnect
            </button>
        </div>
    </div>
    ...
    <div x-show="gatewayStatus !== 'CONNECTED' && gatewayStatus !== 'LOADING' && gatewayStatus !== 'RECONNECTING'" x-cloak
         class="bg-white rounded-md p-10 lg:p-16 shadow-md border border-stone-200 flex flex-col items-center text-center max-w-xl mx-auto mt-8">

        <div class="w-16 h-16 bg-brand-light text-brand-primary rounded-md flex items-center justify-center mb-6 shadow-sm border border-brand-primary/20">
        ...
        <div x-show="gatewayStatus === 'DISCONNECTED' || gatewayStatus === 'OFFLINE'" class="flex flex-col items-center gap-3">
            <button @click="startGateway" :disabled="isStarting"
                class="bg-brand-primary hover:opacity-90 text-white px-8 py-3.5 rounded-md font-extrabold text-[10px] uppercase tracking-widest transition-all shadow-md disabled:opacity-50 flex items-center gap-2">
        ...
        <div x-show="gatewayStatus === 'WAITING_FOR_SCAN'" class="flex flex-col items-center gap-4">
            <div class="bg-stone-50 p-4 rounded-md border border-stone-200 shadow-inner relative">
                <div x-show="qrLoading" class="absolute inset-0 bg-white/80 rounded-md flex items-center justify-center">
                    <i data-lucide="loader-2" class="w-8 h-8 text-brand-primary animate-spin"></i>
                </div>
                <img :src="qrCodeData" alt="WhatsApp QR Code" class="w-60 h-60 object-contain rounded-sm" x-show="qrCodeData">
                <div x-show="!qrCodeData && !qrLoading" class="w-60 h-60 flex flex-col items-center justify-center text-stone-400 bg-white rounded-md gap-2 border border-stone-200">
    ...
    <div x-show="gatewayStatus === 'CONNECTED' || gatewayStatus === 'RECONNECTING'" x-cloak
        ...
        <div class="xl:col-span-2 space-y-5">
            ...
            <div class="bg-white rounded-md border border-stone-200 shadow-sm overflow-hidden">
                <div class="px-7 py-5 border-b border-stone-100 flex items-center justify-between">
                    ...
                    <div class="flex gap-2">
                        <span x-show="selectedCustomers.length > 0" class="px-3 py-1.5 bg-brand-light text-brand-primary text-[10px] font-extrabold uppercase tracking-widest rounded-md tabular-nums">
                ...
                <div class="p-7 space-y-5">
                    ...
                    <div x-data="{ open: false, search: '' }">
                        ...
                        <div class="relative">
                            <div @click="open = !open"
                                 class="w-full flex items-center justify-between bg-stone-50 border border-stone-200 rounded-md px-4 py-3.5 cursor-pointer hover:border-brand-primary/50 transition-all">
                            ...
                            <div x-show="open" @click.outside="open = false"
                                 ...
                                 class="absolute top-full left-0 right-0 mt-2 bg-white border border-stone-200 rounded-md shadow-xl z-30 overflow-hidden">
                                <div class="p-3 border-b border-stone-100">
                                    <input x-model="search" type="text" placeholder="SEARCH TEMPLATES..."
                                           class="w-full text-[10px] font-extrabold uppercase tracking-widest bg-stone-50 border border-stone-200 rounded-md px-3 py-2 outline-none focus:border-brand-primary">
                                </div>
                    ...
                    <div x-data="{ dragging: false }">
                        ...
                        <div 
                            class="relative min-h-[120px] rounded-md border-2 border-dashed transition-all flex flex-col items-center justify-center p-4 group"
                            :class="[
                            ...
                            <div x-show="!selectedImagePreview" class="flex flex-col items-center gap-2 pointer-events-none">
                                <div class="w-10 h-10 bg-white border border-stone-200 rounded-md shadow-sm flex items-center justify-center text-stone-400 group-hover:text-brand-primary transition-colors">
                            ...
                            <div x-show="selectedImagePreview" class="relative w-full flex items-center gap-4 py-2 px-2" x-cloak>
                                <div class="relative w-20 h-20 shrink-0">
                                    <img :src="selectedImagePreview" class="w-full h-full object-cover rounded-md shadow-sm border border-stone-200">
                    ...
                    <div>
                        ...
                        <textarea x-model="messageBody" rows="7"
                            class="w-full bg-stone-50 border border-stone-200 text-stone-900 text-sm rounded-md focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary block p-4 transition-all outline-none resize-none placeholder-stone-400 font-extrabold leading-relaxed"
            ...
            <!-- Broadcast Results -->
            <div x-show="broadcastResults.length > 0" class="bg-white rounded-md border border-stone-200 shadow-sm p-6 mt-5">
                ...
                <div class="max-h-52 overflow-y-auto space-y-1.5 pr-1 custom-scrollbar">
                    <template x-for="(res, i) in broadcastResults" :key="i">
                        <div class="flex items-center justify-between px-4 py-3 bg-stone-50 rounded-md border border-stone-100">
        ...
        <div class="space-y-5">
            <!-- Mode Switcher -->
            <div class="bg-white rounded-md border border-stone-200 shadow-sm p-6">
                ...
                <div class="grid grid-cols-2 gap-2">
                    <button @click="audienceMode = 'manual'; selectedCustomers = []"
                        class="flex flex-col items-center gap-1.5 p-3 rounded-md font-extrabold text-[10px] uppercase tracking-widest transition-all border-2 outline-none"
            ...
            <!-- TAG MODE -->
            <div x-show="audienceMode === 'tag'" class="bg-white rounded-md border border-stone-200 shadow-sm flex flex-col" style="max-height: 540px;">
                <div class="p-5 border-b border-stone-100 shrink-0">
                    ...
                    <div class="flex flex-wrap gap-2 max-h-32 overflow-y-auto custom-scrollbar">
                        ...
                        <button @click="selectTag({{ $tag->id }}, '{{ addslashes($tag->name) }}')"
                            class="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[9px] font-extrabold uppercase tracking-widest transition-all border outline-none"
                ...
                <div class="flex-1 overflow-y-auto p-4 space-y-1.5 custom-scrollbar">
                    ...
                    <div x-show="!tagLoading && tagCustomers.length > 0" class="space-y-1">
                        ...
                        <div @click="toggleSelectAllTagCustomers"
                             class="flex items-center p-3 bg-brand-light rounded-md cursor-pointer border border-brand-primary/20 mb-2">
                        ...
                        <template x-for="(cust, idx) in tagCustomers" :key="cust.id">
                            <label class="flex items-center p-3 hover:bg-stone-50 rounded-md transition-colors cursor-pointer border border-transparent hover:border-stone-100 group">
            ...
            <!-- MANUAL MODE -->
            <div x-show="audienceMode === 'manual'" class="bg-white rounded-md border border-stone-200 shadow-sm flex flex-col" style="max-height: 540px;">
                <div class="p-5 border-b border-stone-100 shrink-0">
                    ...
                    <div class="relative mt-4">
                        ...
                        <input x-model="manualSearch" type="text" placeholder="SEARCH BY NAME OR PHONE..."
                               class="w-full pl-9 pr-3 py-2.5 bg-stone-50 border border-stone-200 rounded-md text-[9px] font-extrabold uppercase tracking-widest focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary outline-none transition-all">
                    </div>
                </div>
                <div class="flex-1 overflow-y-auto p-3 custom-scrollbar">
                    @foreach($customers as $customer)
                        ...
                        <label x-show="manualSearch === '' || '{{ strtolower($customer->name . ' ' . $customer->phone) }}'.includes(manualSearch.toLowerCase())"
                               class="flex items-center p-3 hover:bg-stone-50 rounded-md transition-colors cursor-pointer border border-transparent hover:border-stone-100 group">
            ...
            <div class="bg-white rounded-md border border-stone-200 shadow-sm p-6">
                ...
                <button @click="sendBroadcast" :disabled="selectedCustomers.length === 0 || (!messageBody.trim() && !selectedImageFile) || isSending"
                    class="w-full bg-brand-primary hover:opacity-90 text-white px-6 py-4 rounded-md font-extrabold text-[10px] uppercase tracking-widest transition-all shadow-md disabled:opacity-50 disabled:shadow-none flex items-center justify-center gap-2 outline-none">
